...
const setCurrentProducts = ({result}) => {
  currentProducts = result;  
  const meta={"count":3000,"currentPage":1, "pageCount":0, "pageSize": 12};
...
const fetchProducts = async (page = 1, size = 12) => {
  try {
    const response = await fetch(
      `https://server-rosy-tau.vercel.app/products/search?`
    );
...
const fetchBrands = async () => {
  try {
    ...
    const body = await response.json();
*/
    const body= {"success":true,"data":{"result":["dedicated","montlimar","circlesportswear"]}};
    if (body.success !== true) {
      console.error(body);
      return {brands};
    }
